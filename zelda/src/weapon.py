...
class Weapon(pygame.sprite.Sprite):
    def __init__(self, groups: list[pygame.sprite.Group], event_bus: EventBus):
        ...
        full_path = (
            get_assets_dir() + f"graphics/weapons/{player_weapon}/{direction}.png"
        )
        self.image = pygame.image.load(full_path).convert_alpha()

        if direction == "right":
            self.rect = self.image.get_rect(
                midleft=player_rect.midright + pygame.math.Vector2(0, 16)
            )
        elif direction == "left":
            self.rect = self.image.get_rect(
                midright=player_rect.midleft + pygame.math.Vector2(0, 16)
            )
        elif direction == "up":
            self.rect = self.image.get_rect(
                midbottom=player_rect.midtop + pygame.math.Vector2(-10, 0)
            )
        elif direction == "down":
            self.rect = self.image.get_rect(
                midtop=player_rect.midbottom + pygame.math.Vector2(-10, 0)
            )
        else:
